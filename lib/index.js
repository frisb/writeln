...
var Writeln = function () {
	...
	_createClass(Writeln, [{
		...
		value: function write(level, text, metadata) {
			...
			var log = debug('' + this.formatCategory(level));

			if (level === 'error') log.color = 1;
		...
		key: 'formatCategory',
		value: function formatCategory(level) {
			var str = this.category.replace(/ /g, '-').toLowerCase() + ':' + level;
