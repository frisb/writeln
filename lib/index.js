(function() {
  var Writeln, chalk, custom, dateStr, hr, logger, maxCategoryLength, options, pad, surreal, timeStr, transportOptions, winston;
  ...
  module.exports = Writeln = (function() {
    function Writeln(category, color) {
    ...
    Writeln.prototype.write = function(level, text, metadata) {
      ...
      date = dateStr(now, '-');
      time = timeStr(now);
      text = chalk.dim("" + date + " " + time) + (" | " + (this.formatCategory()) + " | " + text);
      if (metadata) {
    ...
    Writeln.prototype.formatCategory = function() {
      ...
      return str = this.color ? chalk[this.color](str) : str;
    };
